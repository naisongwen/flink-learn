...
public class StickyAllocationAndLocalRecoveryTestJob {
    ...
    public static void main(String[] args) throws Exception {
        ...
        if (pt.getBoolean("externalizedCheckpoints", true)) {
            env.getCheckpointConfig()
                    .enableExternalizedCheckpoints(
                            CheckpointConfig.ExternalizedCheckpointCleanup.RETAIN_ON_CANCELLATION);
        }

        String checkpointDir = pt.get("checkpointDir","file:///./checkpointDir/");
        env.getCheckpointConfig().setCheckpointStorage(checkpointDir);

        boolean killJvmOnFail = pt.getBoolean("killJvmOnFail", false);

        String stateBackend = pt.get("stateBackend", "rocks");
        if ("hashmap".equals(stateBackend)) {
            env.setStateBackend(new HashMapStateBackend());
        } else if ("rocks".equals(stateBackend)) {
            boolean incrementalCheckpoints = pt.getBoolean("incrementalCheckpoints", false);
            EmbeddedRocksDBStateBackend embeddedRocksDBStateBackend=new EmbeddedRocksDBStateBackend(incrementalCheckpoints);
